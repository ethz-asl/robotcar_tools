...
def main():
  ...
  with open(ins_filepath) as extrinsics_file:
      ...
      T_B_RTK = get_T_ENU_NED() * mk.Transformation(build_se3_transform(extrinsics))

  T_B_MF = get_T_ENU_NED() * get_T_B_I()

  T_B_ML = None
  with open(mono_left_filepath) as extrinsics_file:
      extrinsics = [float(x) for x in next(extrinsics_file).split(' ')]
      T_B_ML = get_T_ENU_NED() * mk.Transformation(build_se3_transform(extrinsics)).inverse() * get_T_B_I()

  T_B_MRe = None
  with open(mono_rear_filepath) as extrinsics_file:
      extrinsics = [float(x) for x in next(extrinsics_file).split(' ')]
      T_B_MRe = get_T_ENU_NED() * mk.Transformation(build_se3_transform(extrinsics)).inverse() * get_T_B_I()

  T_B_MRi = None
  with open(mono_right_filepath) as extrinsics_file:
      extrinsics = [float(x) for x in next(extrinsics_file).split(' ')]
      T_B_MRi = get_T_ENU_NED() * mk.Transformation(build_se3_transform(extrinsics)).inverse() * get_T_B_I()
